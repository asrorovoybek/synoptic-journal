...
// --- State Management ---
let siteInfo = JSON.parse(localStorage.getItem('msje_site_info')) || initialSiteInfo;
let announcements = JSON.parse(localStorage.getItem('msje_announcements')) || initialAnnouncements;
let issues = JSON.parse(localStorage.getItem('msje_issues')) || initialIssues;
let articles = JSON.parse(localStorage.getItem('msje_articles')) || initialArticles;
let submissions = JSON.parse(localStorage.getItem('msje_submissions')) || [];
  ...
  form.onsubmit = async (e) => {
    ...
    try {
      ...
      if (file) {
        ...
        const data = await res.json(); filePath = data.filePath;
      }
      ...
      submissions.unshift(newSub); saveState();
...
function attachAdminEvents(path, params) {
  ...
  if (setForm) {
    setForm.onsubmit = (e) => {
      ...
      saveState(); alert('Saqlandi'); handleRoute();
    };
  }
  ...
  if (annForm) {
    annForm.onsubmit = (e) => {
      ...
      saveState(); navigate('/admin/announcements');
    };
  }
  ...
  if (issueForm) {
    issueForm.onsubmit = async (e) => {
      ...
      if (fileInput && fileInput.files[0]) {
        ...
        const res = await fetch('http://localhost:3001/api/upload', { method: 'POST', body: formData });
        const resData = await res.json(); coverPath = resData.filePath;
      }
      ...
      saveState(); navigate('/admin/issues');
    };
  }
  ...
  if (artForm) {
    artForm.onsubmit = async (e) => {
      ...
      try {
        ...
        const art = articles.find(a => a.id === id) || {};
        let pdfPath = id ? (art.pdfPath || 'uploads/sample.pdf') : 'uploads/sample.pdf';
        ...
        if (file) {
          ...
          const resData = await res.json(); pdfPath = resData.filePath;
        }
        ...
        const data = { 
          ...
          issueId: document.getElementById('a-issue').value, 
          doi: document.getElementById('a-doi').value, 
          firstPage: document.getElementById('a-fpage').value,
          lastPage: document.getElementById('a-lpage').value,
          keywords: document.getElementById('a-keywords').value,
          type: document.getElementById('a-type').value, 
          abstract: document.getElementById('a-abstract').value, 
          references: refList, 
          authors: authorList, 
          publicationDate: document.getElementById('a-pdate').value, 
          pdfPath: pdfPath 
        };

        if (id) {
          articles = articles.map(a => a.id === id ? data : a);
        } else {
